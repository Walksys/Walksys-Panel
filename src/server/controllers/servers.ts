    ...
    try {
      ...
      if (!isGeneric) {
        ...
        if (!fs.existsSync(jarPath)) {
          ...
          panelEvents.emit("log", id, `[WALKSYS System] Pre-flight: server.jar not found. Downloading ${server.type} (${server.version || "latest"})...\r\n`);
          try {
            await downloadJar(server.type, server.version || "latest", jarPath);
            panelEvents.emit("log", id, `[WALKSYS System] server.jar downloaded successfully.\r\n`);
          } catch (dlErr: any) {
            panelEvents.emit("log", id, `[WALKSYS System] Notice: Automatic JAR download error: ${dlErr?.message || dlErr}\r\n`);
          }
        }
        const eulaPath = path.join(serverDir, "eula.txt");
        if (!fs.existsSync(eulaPath)) {
...
export const redownloadJar = async (req: Request, res: Response) => {
  ...
  try {
    const { panelEvents } = await import("../events.js");
    panelEvents.emit("log", id, `[WALKSYS System] Downloading ${server.type} (${server.version || "latest"}) server JAR...\r\n`);
    await downloadJar(server.type, server.version || "latest", jarPath);
    await secureExecutablePermissions(jarPath);
    ...
    if (server.runtimeType !== "local" && server.containerId) {
      try {
        const status = await getServerRuntimeStatus(server);
        if (!status?.State?.Running) {
          panelEvents.emit("log", id, `[WALKSYS System] Refreshing Docker container environment...\r\n`);
          await deleteServerRuntime(server);
          server.containerId = await createServerRuntime(server);
          await writeJSON("servers.json", servers);
        }
    ...
    panelEvents.emit("log", id, `[WALKSYS System] Server JAR successfully installed and configured!\r\n`);
...
export const installPlugin = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const commonHeaders = {
      'User-Agent': 'WALKSYSPanel/3.1.0 (https://github.com/Walksys; support@jtgpanel.net)'
    };
