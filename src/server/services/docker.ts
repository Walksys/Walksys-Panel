...
export const createServerContainer = async (serverData: any, nodeId?: string) => {
  ...
  const containerBindPath = isLocal ? serverDir : `/opt/Walksys-panel-node/servers/${serverData.id}`;
  ...
  const buildContainerOptions = (img: string) => {
    ...
    return {
      Image: img,
      name: `Walksys-server-${serverData.id}`,
      Tty: true,
      OpenStdin: true,
      StdinOnce: false,
      Env: envVars,
  ...
  try {
    ...
    if (err?.statusCode === 409 || errStr.includes("409") || errStr.includes("Conflict") || errStr.includes("already in use")) {
      console.log(`Container name collision for Walksys-server-${serverData.id}. Removing stale container...`);
      try {
        const oldCont = docker.getContainer(`Walksys-server-${serverData.id}`);
        ...
        if (["NODEJS", "NODE"].includes(type)) {
          ...
          if (!fs.existsSync(pkgPath)) {
            await fs.writeFile(pkgPath, JSON.stringify({
              ...
              version: "1.0.0",
              description: "Node.js app on WALKSYS Panel",
              main: "index.js",
  ...
  try {
    ...
    if (server) {
      ...
      if (!isGeneric) {
        const jarPath = path.join(serverDir, "server.jar");
        if (!fs.existsSync(jarPath)) {
          panelEvents.emit("log", server.id, `[WALKSYS System] Downloading ${server.type} (${server.version || "latest"}) server JAR...\r\n`);
          try {
            await downloadJar(server.type, server.version || "latest", jarPath);
            panelEvents.emit("log", server.id, `[WALKSYS System] Server JAR downloaded successfully.\r\n`);
          } catch (err: any) {
            panelEvents.emit("log", server.id, `[WALKSYS System] Warning: JAR download error: ${err?.message || err}\r\n`);
          }
        }
        const eulaPath = path.join(serverDir, "eula.txt");
        if (!fs.existsSync(eulaPath)) {
