...
router.get("/health", (req, res) => {
  res.json({ status: "ok", panel: "WALKSYS Panel", version: "3.2.0" });
});
...
router.post("/webhook/github-update", async (req, res) => {
  ...
  console.log("[WALKSYS Panel] GitHub push webhook triggered! Initiating automatic panel update...");
  res.json({ success: true, message: "Automatic update triggered from GitHub push." });

  setTimeout(() => {
    exec("bash update.sh", (error, stdout, stderr) => {
      if (error) {
        console.error(`[WALKSYS Panel Auto-Update Error]:`, error);
      }
      console.log(`[WALKSYS Panel Auto-Update Output]:\n${stdout}`);
    });
  }, 1000);
});
...
router.get("/settings", async (req, res) => {
  ...
  res.json({ 
    panelName: settings.panelName || "WALKSYS Panel",
